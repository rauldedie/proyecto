...
if (isset($_POST["email"]) && (!empty($_POST["email"])))
{
    ...
    {
        $sel_query = "SELECT * FROM usuarios WHERE mail='".$emailc."'";
        $results = mysqli_query($bdcon,$sel_query);
    ...
    }else
    {
        ...
        $key = $key . $addKey;
        // Insert Temp Table
        mysqli_query($bdcon, "INSERT INTO `password_reset_temp` (`email`, `key`, `expDate`) VALUES ('".$emailc."', '".$key."', '".$expDate."');");
        $output='<p>Estimado usuario,</p>';
        $output.='<p>Por favor haz click en el enlace de abajo para resetear tu contraseña.</p>';
        $output.='<p>-------------------------------------------------------------</p>';
        $output.='<p><a href="https://practicasrdm.es/include/reset-password.php?key='.$key.'&email='.$email.'&action=reset" target="_blank">https://http://practicasrdm.es/include/reset-password.php?key='.$key.'&email='.$email.'&action=reset</a></p>';		
        $output.='<p>-------------------------------------------------------------</p>';
