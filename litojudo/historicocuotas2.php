...
if(isset($_GET['idalumno']))
{
    ...
    //dibujo tabla para mostrar los datos del elto alumno y sus cuotas
           ...
           // Usar switch para obtener el nombre del mes
            ...
            /*echo "<tr>
              ...
              echo "<tr>
                      ...
                      <td>
                          <span id='estado-{$fila['mes']}-{$fila['anio']}'>" . htmlspecialchars($estado, ENT_QUOTES, 'UTF-8') . "</span>
                          <button class='btn btn-primary btn-sm ml-2' onclick='cambiarEstado({$id}, {$fila['mes']}, {$fila['anio']}, {$fila['estado']})'>$botonTexto</button>
                      </td>
                    </tr>";  
        }
        ...
        echo "
            <script>
                function cambiarEstado(idAlumno, mes, anio, estadoActual) {
                    ...
                    body: 'idAlumno=' + idAlumno + '&mes=' + mes + '&anio=' + anio + '&nuevoEstado=' + nuevoEstado
                })
                    .then(response => response.json())
                    .then(data => {
                    if(data.success) {
                    let estadoSpan = document.getElementById('estado-' + mes + '-' + anio);
                    let boton = estadoSpan.nextElementSibling;
